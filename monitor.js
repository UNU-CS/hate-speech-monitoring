...
var config = {
  ...
  post_export_file: './monitor.export',
  comments_export_file: './monitor.export',
...
function format_post(post, source_id) {
  return {
    source_id: source_id,
    id: post.id,
...
function format_comment(comment, post_id) {
  return {
    post_id: post_id,
    id: comment.id,
...
function get_comments(post_id, after) {
...
function get_posts(source_id) {
  get('/' + source_id + '/feed', post_params, null, function(res) {
    ...
    var post_records = _.map(res.data, format_post);
...
// Caution: this will create duplicate comments when run twice or when these
// posts "die."
function save_live_post_comments() {
  _.each(state.live_posts, get_all_comments);
}

// Remove duplicate comments in case of injudicious use of 
// save_live_post_comments
function clean_comments() {
  unique_comments = _.uniq(state.comments, function(comment) {
    return comment.id;
  });
  console.log('Found ' + (state.comments.length - unique_comments.length) + ' duplicate comments.');
  state.comments = unique_comments;
}
...
function to_csv(arr) {
  ...
  return json2csv({ data: arr, fields: _.keys(arr[0]) });
}
...
function export_all() {
  save_live_post_comments();
  clean_comments();
  save(to_csv(state.posts), config.post_export_file);
  save(to_csv(state.comments), comment_export_file);
}
...
function test_sources() {
  var posts = 0;
  async.each(state.sources, function(source_id, callback) {
    get('/' + source_id + '/feed', post_params, null, function(res) {
    ...
    if(err) {
      ...
      console.log('Successfully got posts (' + posts + ') from all sources (' + state.sources.length + ').');
      if(state.sources.length * config.post_limit !== posts) {
        console.log('Warning: hoped to get ' + (state.sources.length * config.post_limit) + ' posts, but only got ' + posts + '.');
      }
    }
  });
}
